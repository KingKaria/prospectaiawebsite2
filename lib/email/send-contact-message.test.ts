...
describe("sendContactMessage — segurança dos campos de envelope", () => {
  it("usa sempre o sender e o to do ambiente, controlados pelo servidor", async () => {
    ...
    const [, requestInit] = fetchMock.mock.calls[0];
    const payload = JSON.parse(requestInit.body);
    expect(payload.sender).toEqual({ email: FROM_EMAIL, name: FROM_NAME });
    expect(payload.to).toEqual([{ email: TO }]);
  });

  it("usa o email validado do visitante apenas como replyTo, nunca como sender", async () => {
    fetchMock.mockResolvedValue(jsonResponse(201, { messageId: "x" }));
    await sendContactMessage(validInput);
    const [, requestInit] = fetchMock.mock.calls[0];
    const payload = JSON.parse(requestInit.body);
...
describe("sendContactMessage — erros", () => {
  it("devolve send_error quando o Brevo responde com um estado não 2xx", async () => {
  ...
  it("devolve send_error quando o corpo de erro não é JSON válido", async () => {
    fetchMock.mockResolvedValue(
  ...
  it("devolve send_error quando o fetch rejeita por erro de rede, sem propagar a exceção", async () => {
    fetchMock.mockRejectedValue(new Error("stack trace interno"));
    const result = await sendContactMessage(validInput);
    expect(result).toEqual({ ok: false, reason: "send_error" });
  });

  it("devolve send_error quando o fetch rejeita por timeout (AbortSignal.timeout)", async () => {
    // O mesmo tipo de erro que AbortSignal.timeout() produz quando o
    // pedido excede REQUEST_TIMEOUT_MS — testado explicitamente, não só
    // por analogia com uma rejeição genérica.
...
describe("sendContactMessage — trim seguro da configuração", () => {
  it("string undefined em qualquer variável → missing_config, sem chamar o Brevo", async () => {
    delete process.env.BREVO_API_KEY;
    expect(await sendContactMessage(validInput)).toEqual({ ok: false, reason: "missing_config" });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("string vazia em qualquer variável → missing_config, sem chamar o Brevo", async () => {
    process.env.CONTACT_EMAIL_FROM = "";
    expect(await sendContactMessage(validInput)).toEqual({ ok: false, reason: "missing_config" });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("string só com espaços em qualquer variável → missing_config, sem chamar o Brevo", async () => {
  ...
  it("string com espaços à volta de um valor válido não bloqueia — é aparada antes de usar", async () => {
    ...
    expect(result).toEqual({ ok: true });
    const [, requestInit] = fetchMock.mock.calls[0];
    expect(JSON.parse(requestInit.body).to).toEqual([{ email: TO }]);
  });

  it("as quatro variáveis válidas e sem espaços → chama o Brevo (fetch)", async () => {
    fetchMock.mockResolvedValue(jsonResponse(201, { messageId: "x" }));
    await sendContactMessage(validInput);
...
describe("sendContactMessage — logging sem PII", () => {
  ...
  it("nenhuma chamada real à Brevo — fetch está sempre mockado", () => {
